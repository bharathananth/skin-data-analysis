...
time_D <- (experiment %>% arrange(subject) %>% filter(tissue=="D") %$% time)/24 #rearrange time
time_D <- ifelse(time_D>1, time_D-1, time_D)
time_E <- (experiment %>% arrange(subject) %>% filter(tissue=="E") %$% time)/24 
...
suppfig5A <- ggplot(cvResultGathGroup) + facet_wrap(~tissue, scales='free') + theme_custom() +
  ...
  scale_y_continuous(limits=c(0.025, 0.125)) + scale_x_continuous(breaks=c(1,2,3,4,5,6)) + expand_limits(x=c(1,6)) +
...
spcResultFinal_D <- zeitzeigerSpc(fitResultFinal_D$xFitMean, fitResultFinal_D$xFitResid, 
                                  sumabsv=sumabsv_D) #sumabsv=3 + 2SPCs gives low MAE in D (doesn't improve much after)
spcResultFinal_E <- zeitzeigerSpc(fitResultFinal_E$xFitMean, fitResultFinal_E$xFitResid, 
                                  sumabsv=sumabsv_E) #sumabsv=3 + 2SPCs gives least MAE in E (doesn't improve much after)
...
suppfig5B <- ggplot(dfVar) + facet_wrap(~tissue, scales='free') + scale_y_continuous(limits=c(0.0,0.7)) +
  ...
  theme_custom() + theme(legend.title = element_text(),
                         ...
                         axis.line=element_line()) #first 3 SPCs explain variance, 
                                                   #but error is not improved much after 2SPCs (see suppfig5A)


# Plot the phase portrait of SPCs over time: Supplementary Figure 5D
# ------------------------------------------------------------------
...
# Plot coefficients of the features (time-telling genes) for the SPCs: Supplementary Figure 5C
# --------------------------------------------------------------------------------------------
...
suppfig5C_1 <- ggplot(vGath %>% filter(tissue=="dermis")) + facet_wrap(~spc, scales="free") +
  ...
  theme(axis.ticks = element_blank(),
        ...
        strip.background = element_rect(fill=alpha("#1B9E77", 0.5), linetype = 'dashed'),
        strip.text = element_text(size=16),
        aspect.ratio=5) + ggtitle(paste0("dermis, sumabsv=", sumabsv_D)) +
  scale_size(limits = c(NA, NA), range = c(2, 6.)) + guides(fill=FALSE) 
# https://stackoverflow.com/questions/63393553/color-legend-key-labels-with-r-ggplot2-and-remove-the-keys

suppfig5C_2 <- ggplot(vGath %>% filter(tissue=="epidermis")) + facet_wrap(~spc, scales="free") +
  geom_label(aes(x=spc, y=feature, label=Symbol_it, size=Coefficient, color=sign, fill=varPart_gene), 
             label.size=NA, parse=TRUE) + 
  scale_color_manual(values=c("#d11141", "steelblue3")) + scale_fill_manual(values=c("transparent", "#ffe62f")) + 
  theme_custom() + labs(size="SPC coefficient\n(absolute)") + 
  theme(axis.ticks = element_blank(),
        ...
        panel.border = element_rect(colour = "black", fill=NA, linetype = 'dashed'),
        legend.title = element_text(face="bold"),
        strip.background = element_rect(fill=alpha("#D95F02", 0.5), linetype = 'dashed'),
        strip.text = element_text(size=16),
        aspect.ratio=5) + ggtitle(paste0("epidermis, sumabsv=", sumabsv_E)) +
  scale_size(limits = c(NA, NA), range = c(2, 6.)) + guides(fill=FALSE) 

suppfig5C <- ggpubr::ggarrange(suppfig5C_1, NULL, suppfig5C_2, nrow=1, ncol=3, 
                               common.legend=TRUE, legend="right", widths=c(1.,0.1,1))
...
# Plot timeseries of time-telling genes: Supplementary figure 5E
# --------------------------------------------------------------
...
suppfig5E_1 <- ggplot(yGath_D) + geom_line(aes(x=time, y=expression, color=subject)) + 
  facet_wrap(~Symbol, scales="free", ncol=7, nrow=8) + xlab("internal time") + ylab(bquote(~log[2]*'expression (normalized)')) +
  theme_custom() + theme(strip.text = element_text(face="bold.italic"),
                         legend.position="right",
                         legend.title=element_text(),
                         strip.background = element_rect(fill=alpha("#1B9E77", 0.5), linetype = 'dashed'),
                         panel.border = element_rect(fill="transparent", linetype = 'dashed'),
                         axis.line = element_blank(),
...
suppfig5E_2 <- ggplot(yGath_E) + geom_line(aes(x=time, y=expression, color=subject)) + 
  facet_wrap(~Symbol, scales="free", ncol=7, nrow=8) + xlab("internal time") + ylab(bquote(~log[2]*'expression (normalized)')) +
  theme_custom() + theme(strip.text = element_text(face="bold.italic"),
                         legend.position="right",
                         legend.title=element_text(),
...
suppfig5E <- ggpubr::ggarrange(suppfig5E_1, NULL, suppfig5E_2, ncol=3, nrow=1, 
...
# ---------------------
sfig5_1 <- plot_grid(suppfig5A, NULL, suppfig5B, labels=c("", "", "B"), nrow=1, ncol=3, rel_widths=c(1,0.05,0.85),
                     align="h", axis="b")
sfig5_2 <- plot_grid(suppfig5C, suppfig5D, labels=c("", "D"), ncol=2, nrow=1, rel_widths=c(2, 1))

sfig5_part1 <- plot_grid(sfig5_1, NULL, sfig5_2, nrow=3, ncol=1, rel_heights=c(0.75,0.1,1.5,0.1), 
                         labels=c("A", "", "C"), align="v", axis="l")
sfig5_part2 <- plot_grid(suppfig5E_1, NULL, suppfig5E_2, nrow=3, ncol=1, rel_heights=c(3.,0.1,3.*0.625), 
                         labels="E", align="v", axis="l")

sfig5_part1 %>% ggsave('figures/suppfig5_part1.pdf', ., width = 11, height = 9.5)
sfig5_part2 %>% ggsave('figures/suppfig5_part2.pdf', ., width = 11, height = 20.5)
