"
Internal time is calculated as wall time minus the mid-sleep time MSF_sc
"
...
experiment <- experiment %>% full_join(info_subjects)
...
# 6.2. Crossvalidation internal time
# ----------------------------------
...
# Correct wall time to internal time
df_time_D = data.frame(subject = sapply(strsplit(rownames(xD), split="_"), "[", 1),
                     wall_time = str_sub(sapply(strsplit(rownames(xD), split="_"), "[", 2), start=2) %>% as.numeric()) %>%
  inner_join(info_subjects %>% dplyr::select(subject, MSF_sc)) %>%
  dplyr::mutate(MSF_sc_dec = hms(MSF_sc)) %>%
  dplyr::mutate(MSF_sc_dec = round((hour(MSF_sc_dec) + minute(MSF_sc_dec) / 60 + second(MSF_sc_dec) / 360),2),
                internal_time = wall_time + MSF_sc_dec,
                internal_time_norm = internal_time/24,
                
                diff_to_refsubj = MSF_sc_dec - median(MSF_sc_dec),
                internal_time2 = wall_time + diff_to_refsubj,
                internal_time2_norm = internal_time2/24)
time_D = ifelse(df_time_D$internal_time_norm>1, df_time_D$internal_time_norm - 1, df_time_D$internal_time_norm)

df_time_E = data.frame(subject = sapply(strsplit(rownames(xE), split="_"), "[", 1),
                       wall_time = str_sub(sapply(strsplit(rownames(xE), split="_"), "[", 2), start=2) %>% as.numeric()) %>%
  inner_join(info_subjects %>% dplyr::select(subject, MSF_sc)) %>%
  dplyr::mutate(MSF_sc_dec = hms(MSF_sc)) %>%
  dplyr::mutate(MSF_sc_dec = round((hour(MSF_sc_dec) + minute(MSF_sc_dec) / 60 + second(MSF_sc_dec) / 360),2),
                internal_time = wall_time + MSF_sc_dec,
                ...
                internal_time2 = wall_time + diff_to_refsubj,
                internal_time2_norm = internal_time2/24)
time_E = ifelse(df_time_E$internal_time_norm>1, df_time_E$internal_time_norm - 1, df_time_E$internal_time_norm)
...
spcResultFinal_D = zeitzeigerSpc(fitResultFinal_D$xFitMean, fitResultFinal_D$xFitResid, 
                                 sumabsv = sumabsv_D) #sumabsv=1 + 2SPCs gives least MAE in D
spcResultFinal_E = zeitzeigerSpc(fitResultFinal_E$xFitMean, fitResultFinal_E$xFitResid, 
                                 sumabsv = sumabsv_E) #sumabsv=3 + 5SPCs gives least MAE in E
...
ggplot(dfVar) + facet_wrap(~tissue) +
  ...
  labs(x = 'SPC', y = 'Proportion of\nvariance explained') + theme_bw() #only first 3 SPCs explain variance
                                                                        #but error is not improved signif. after 2SPCs
...
colnames(zD) = c('SPC 1', 'SPC 2'); colnames(zE) = colnames(zD)
...
pdf("figures/fig2_ZZ_internal.pdf")
for (i in 1:2){
  print(ggplot(vGath) + facet_grid_paginate(tissue ~ spc, ncol=2, nrow=1, page=i) +
          geom_bar(aes(x = Symbol, y = Coefficient, fill=tissue), stat = 'identity') +
          labs(x = 'Feature') + coord_flip() +
          theme_bw() + ggtitle(paste0("ZeitZeiger internal time, int_time = wall_time + mid_sleep_time\nsumabsv_dermis=", 
                                      sumabsv_D, ", sumabsv_epidermis=", sumabsv_E)) +
...
#  labs(x = 'Feature') + coord_flip() +
#  theme_bw() + ggtitle(paste0("ZeitZeiger on dermis, sumabsv=", sumabsv_D)) +
#  theme(panel.spacing = unit(1.2, 'lines'),
#        axis.text.y = element_text(face="italic", size=6),
#        axis.line = element_line(colour = "black"),
#        panel.border = element_blank(),
...
#  labs(x = 'Feature') + coord_flip() +
#  theme_bw() + ggtitle(paste0("ZeitZeiger on epidermis, sumabsv=", sumabsv_E)) +
#  theme(panel.spacing = unit(1.2, 'lines'),
#        axis.text.y = element_text(face="italic", size=6),
#        axis.line = element_line(colour = "black"),
#        panel.border = element_blank(),
