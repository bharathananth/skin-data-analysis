...
time_D[time_D > 1] = time_D[time_D > 1] - 1; time_E[time_E > 1] = time_E[time_E > 1] - 1
#time_D = (time_D - min(time_D))/24; time_E = (time_E - min(time_E))/24
...
spcResultFinal_D = zeitzeigerSpc(fitResultFinal_D$xFitMean, fitResultFinal_D$xFitResid, 
                                 sumabsv = sumabsv_D) #sumabsv=3 + 3nSPCs gives least MAE in D
spcResultFinal_E = zeitzeigerSpc(fitResultFinal_E$xFitMean, fitResultFinal_E$xFitResid, 
                                 sumabsv = sumabsv_E) #sumabsv=3 + 4nSPCs gives least MAE in E
...
pdf("figures/fig2_ZZ_walltime.pdf")
for (i in 1:2){
  print(ggplot(vGath) + facet_grid_paginate(tissue ~ spc, ncol=2, nrow=1, page=i) +
    ...
    theme_bw() + ggtitle(paste0("ZeitZeiger\nsumabsv_dermis=", sumabsv_D, ", sumabsv_epidermis=", sumabsv_E)) +
